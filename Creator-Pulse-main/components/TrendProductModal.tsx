...
export function TrendProductModal({ isOpen, onClose, product, onPageChange }: TrendProductModalProps) {
  ...
  // Hooks must run on every render in the same order
  const videoEmbedUrl = useMemo(() => {
    const id = product?.videoId;
  ...
  useEffect(() => {
    if (!isOpen || !product?.videoId) return;
    setLoading(true);
    fetch(`https://${location.host.includes('localhost') ? 'jprvfwkayhzfqqwbhold.supabase.co' : 'jprvfwkayhzfqqwbhold.supabase.co'}/functions/v1/make-server-e8bed437/trends/video-details?videoId=${encodeURIComponent(product.videoId)}`)
      .then(r => r.json())
      .then(setDetails)
    ...
    <Dialog open={isOpen} onOpenChange={handleOpenChange}>
      ...
      >
        ...
        <div className="space-y-6">
            ...
            <Card>
              ...
              <CardContent className="space-y-3">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <span className="font-semibold">{Number(product.views||0).toLocaleString()}</span>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-sm text-muted-foreground">예상 수익</span>
                  <span className="font-semibold text-green-600">₩{Number(product.revenue||0).toLocaleString()}</span>
                </div>
                <div className="flex items-center justify-between">
            ...
            <Card>
              ...
              <CardContent className="space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  <span className="font-semibold text-green-600">{details?.forecast?.nextWeek ?? detailedData.forecast.nextWeek}</span>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-sm text-muted-foreground">다음 달 예상</span>
                  <span className="font-semibold text-green-600">{details?.forecast?.nextMonth ?? detailedData.forecast.nextMonth}</span>
                </div>
                <div className="space-y-2">
                  <div className="flex items-center justify-between">
                    <span className="text-sm text-muted-foreground">신뢰도</span>
                    <span className="font-semibold">{details?.forecast?.confidence ?? detailedData.forecast.confidence}%</span>
                  </div>
                  <Progress value={details?.forecast?.confidence ?? detailedData.forecast.confidence} className="h-2" />
                </div>
              </CardContent>
            </Card>
          </div>
          ...
          <Card>
            ...
            <CardContent>
              <div className="space-y-3">
                {(details?.competitorGrowth ?? detailedData.competitorGrowth).map((competitor: any, index: number) => (
          ...
          <Card>
            ...
            <CardContent>
              <div className="space-y-3">
                {(details?.topCreators ?? detailedData.topCreators).map((creator: any, index: number) => (
                  <div key={index} className="flex items-center justify-between p-3 border rounded-lg">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-muted-foreground">구독자 {typeof creator.subscribers === 'number' ? creator.subscribers.toLocaleString() : creator.subscribers}</p>
                      </div>
                    </div>
                    <div className="text-right">
          ...
          <Card>
            ...
            <CardContent>
              <div className="space-y-2">
                {(details?.insights ?? detailedData.insights).map((insight: string, index: number) => (
      ...
      {videoEmbedUrl && (
        <div className="fixed inset-0 flex items-center justify-center pointer-events-none">
          <div className="pointer-events-auto w-[70vw] max-w-[960px] bg-black rounded-lg overflow-hidden shadow-2xl hidden" />
        </div>
      )}
